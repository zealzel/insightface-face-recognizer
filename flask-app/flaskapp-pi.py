...
def gen_frames():
    while True:
        ...
        # Use insightface to detect faces in the frame (BGR format)
        faces = fa.get(frame)
        for face in faces:
            ...
            # Draw the face bounding box using cv2.rectangle
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)
            cv2.rectangle(
                frame, (left, bottom - 35), (right, bottom), (0, 0, 255), cv2.FILLED
            )

            # Save text and position for later drawing with PIL
